// Requires: pinput (add to pubspec.yaml: pinput: ^3.0.0 or latest)
// Requires: http (add to pubspec.yaml: http: ^0.13.3 or latest)
...
class _VerifyCodeScreenState extends State<VerifyCodeScreen> {
  ...
  // It's a good practice to move API configuration to a separate file.
  static const String _baseUrl = 'http://10.254.173.93:3000';
  ...
  Future<void> _resendOtp() async {
    ...
    try {
      print('Sending OTP to: $_baseUrl/send-otp');
  ...
  Future<String?> _verifyOtp(String otp) async {
    try {
      print('Verifying OTP at: $_baseUrl/verify-otp');
      final response = await http.post(
        Uri.parse('$_baseUrl/verify-otp'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({'email': widget.email, 'otp': otp.trim()}),
      );
      print('Response status: ${response.statusCode}');
      print('Response body: ${response.body}');
      ...
      // Return server error message or a default one
      return data['error'] ?? 'An unknown verification error occurred.';
    } catch (e) {
  ...
  Future<void> _onVerifyPressed() async {
    ...
    if (errorMessage == null) {
      ...
      try {
        ...
        _showSnackBar('Registration failed: ${e.message ?? "An unknown error occurred."}');
      }
    } else {
